...
import { X, Search, MapPin, Plus, Loader2 } from "lucide-react"
...
export function AddPlaceModal({ listId, onClose, onPlaceAdded }: AddPlaceModalProps) {
  ...
  const handleAddPlace = async () => {
    if (!selectedPlace || !listId || !dbUser) return

    try {
      setIsAdding(true)
      console.log(`Adding place to list ${listId}:`, selectedPlace)

      // First, check if the place already exists in the database
      const checkResponse = await fetch(`/api/places?lat=${selectedPlace.lat}&lng=${selectedPlace.lng}`)
      ...
      if (checkResponse.ok) {
        ...
        toast({
          title: "Place added",
          description: `${selectedPlace.name} has been added to your list.`,
        })
        ...
        <form onSubmit={handleSubmit} className="p-4">
          {!selectedPlace ? (
            <>
              ...
              {searchError && <div className="mb-4 p-3 bg-red-50 text-red-700 text-sm rounded-md">{searchError}</div>}
